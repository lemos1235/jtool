...
public class XxTenantSqlParser extends AbstractJsqlParser {
    ...
    public void processInsert(Insert insert) {
        Optional<Column> any = insert.getColumns()
                .stream().filter(i -> i.toString().equals(CommonConstant.DB_TENANT_ID)).findAny();
        if (!any.isPresent()) {
    ...
    protected Expression builderExpression(Expression currentExpression, Table table) {
        final Expression tenantExpression = getTenantHandler().getTenantId( Objects.nonNull(currentExpression) );
